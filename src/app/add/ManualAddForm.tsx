...
type Props = {
  onBack: () => void; // назад к поиску
  onDone: () => void; // после успешного сохранения
};

export function ManualAddForm({ onBack, onDone }: Props) {
  ...
  function onSeasonBlur(index: number) {
    setSeasons((prev) => {
      const next = [...prev];
      next[index] = { ...next[index], episodesCount: normalizeEpisodesCount(next[index].episodesCount) };
      return next;
    });
  }
        ...
        {seasons.map((s, idx) => (
          <FormField
            ...
            onKeyDown={(e) => {
              ...
              const next = document.querySelector<HTMLInputElement>(`[data-name="season-${idx + 1}"]`);
              if (next) next.focus();
              else {
                const save = document.querySelector<HTMLButtonElement>(`[data-name="save-button"]`);
                save?.focus();
              }
            }}
          />
